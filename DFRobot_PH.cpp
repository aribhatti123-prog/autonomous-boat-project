...
#define EEPROM_write(address, p) {unsigned int i = 0; uint8_t *pp = (uint8_t*)&(p);for(; i < sizeof(p); i++) EEPROM.write(address+i, pp[i]);}
#define EEPROM_read(address, p)  {unsigned int i = 0; uint8_t *pp = (uint8_t*)&(p);for(; i < sizeof(p); i++) pp[i]=EEPROM.read(address+i);}
...
uint8_t DFRobot_PH::cmdParse(const char* cmd)
{
    uint8_t modeIndex = 0;
...
void DFRobot_PH::phCalibration(uint8_t mode)
{
